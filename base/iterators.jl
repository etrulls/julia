...
import Base:
    start, done, next, isempty, length, size, indices, ndims, last, first,
...
"""
    Iterators.IndexValue(values, keys) <: Associative{eltype(keys), eltype(values)}

Transforms an indexable container into an Associative-view of the same data.
Modifying the key-space of the underlying data may invalidate this object.
"""
struct IndexValue{K, V, I, A} <: Associative{K, V}
    data::A
    itr::I
...
# pairs(v::IndexValue) = v # listed for reference (but already defined by being an Associative)
